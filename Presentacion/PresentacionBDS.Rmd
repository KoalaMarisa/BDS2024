---
title: "PresentacionBDS"
author: "Maria Isabel Gamez"
date: "`r Sys.Date()`"
...
* Los datos se han obtenido a partir de los resultados de \Sexpr{nrow(m8datos_limpios)} encuestas de las \Sexpr{length(unique(m8datos_limpios$year))} ediciones de la [European Social Survey](https://www.europeansocialsurvey.org/) en \Sexpr{length(unique(m8datos_limpios$cntry))} países de Europa. 

* Estas encuestas han tenido lugar desde \Sexpr{min(m8datos_limpios$year)} hasta \Sexpr{max(m8datos_limpios$year)}.
...
# Calcular la felicidad media por país y año
felicidad_media <- m8datos_limpios %>%
  group_by(cntry, year) %>%
  summarise(mean_happy = mean(happy, na.rm = TRUE), .groups = 'drop')

# Imprimir la tabla en la consola de RStudio
print(felicidad_media)

# Generar la tabla con formato y desplazable
...
1. Mapa confianza en la policía
2. Correlaciones "posicionamiento político" y "confianza en la policía"
